...
final class EntraIdAppOnlyAuthenticationContext implements IAuthenticationContext
{
    ...
    /**
     * Discard the cached token so the next authenticateRequest() re-acquires
     * one. Used by SharePointConnector::withSharePointRetry() to recover from
     * a transient Entra ID failure without rebuilding the whole context.
     */
    ...
    /**
     * Unix timestamp the cached token expires at, or 0 if none acquired yet.
     */
